...
describe('installUtils', () => {
  ...
  it('deve instalar utils quando o diretório de destino é fornecido como parâmetro', async () => {
    // Mock para existsSync retornar true para um caminho específico
    (fs.existsSync as jest.Mock).mockImplementation((path) => {
      return path.includes('dist/utils');
    });

    // Chamar a função com um diretório de destino
    await installUtils('./test-utils');

    // Verificar se as funções corretas foram chamadas
    expect(fs.existsSync).toHaveBeenCalled();
    expect(fs.ensureDirSync).toHaveBeenCalled();
    expect(fs.copySync).toHaveBeenCalled();
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Instalando funções utilitárias'));
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Funções utilitárias instaladas com sucesso'));
  });
  ...
  it('deve perguntar pelo diretório de destino quando não é fornecido como parâmetro', async () => {
    ...
    (input as jest.Mock).mockResolvedValue('./test-utils');
  ...
  it('deve sair quando o usuário cancela a instalação', async () => {
    ...
    await expect(installUtils()).rejects.toThrow('Process.exit(0)');
  ...
  it('deve sair com erro quando as utils não são encontradas', async () => {
    ...
    await expect(installUtils('./test-utils')).rejects.toThrow('Process.exit(1)');

    // Verificar se as funções corretas foram chamadas
    expect(fs.existsSync).toHaveBeenCalled();
    expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Funções utilitárias não encontradas'));
