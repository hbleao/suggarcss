...
describe('installHooks', () => {
  ...
  it('deve instalar hooks quando o diretório de destino é fornecido como parâmetro', async () => {
    // Mock para existsSync retornar true para um caminho específico
    (fs.existsSync as jest.Mock).mockImplementation((path) => {
      return path.includes('dist/hooks');
    });

    // Chamar a função com um diretório de destino
    await installHooks('./test-hooks');

    // Verificar se as funções corretas foram chamadas
    expect(fs.existsSync).toHaveBeenCalled();
    expect(fs.ensureDirSync).toHaveBeenCalled();
    expect(fs.copySync).toHaveBeenCalled();
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Instalando hooks'));
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Hooks instalados com sucesso'));
  });
  ...
  it('deve perguntar pelo diretório de destino quando não é fornecido como parâmetro', async () => {
    ...
    (input as jest.Mock).mockResolvedValue('./test-hooks');
  ...
  it('deve sair quando o usuário cancela a instalação', async () => {
    ...
    await expect(installHooks()).rejects.toThrow('Process.exit(0)');
  ...
  it('deve sair com erro quando os hooks não são encontrados', async () => {
    ...
    await expect(installHooks('./test-hooks')).rejects.toThrow('Process.exit(1)');

    // Verificar se as funções corretas foram chamadas
    expect(fs.existsSync).toHaveBeenCalled();
    expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Hooks não encontrados'));
